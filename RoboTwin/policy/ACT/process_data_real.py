...
if __name__ == "__main__":
    ...
    python process_data_real.py \
      /home/zhangw/UR5e_DataCollection/RoboTwin_like_data/run_20251130_194629 \
      torch_cube \
      simple \
      1
    """
    ...
    parser = argparse.ArgumentParser(
        description="Process real-world RoboTwin_like_data episodes for ACT."
    )
    parser.add_argument(
        "run_root",
        type=str,
        help="RoboTwin_like_data 下某次采集的 run_XXXX 目录的完整路径",
    )
    parser.add_argument(
        "task_name",
        type=str,
        help="任务名，例如 torch_cube",
    )
    parser.add_argument(
        "task_config",
        type=str,
        help="任务配置名，例如 simple",
    )
    parser.add_argument(
        "expert_data_num",
        type=int,
        help="用于训练的 episode 条数",
    )

    args = parser.parse_args()

    run_root = args.run_root          # e.g. /home/.../RoboTwin_like_data/run_20251130_194629
    task_name = args.task_name        # e.g. torch_cube
    task_config = args.task_config    # e.g. simple
    expert_data_num = args.expert_data_num

    # 真实 hdf5 的所在目录：run_xxxx/torch_cube/simple/data
    raw_data_dir = os.path.join(run_root, task_name, task_config, "data")
    print("使用原始数据目录:", raw_data_dir)
    ...
    config_key = f"sim-{task_name}-{task_config}-{expert_data_num}"
    SIM_TASK_CONFIGS[config_key] = {
    ...
    print("写入 SIM_TASK_CONFIGS.json 中的 key:", config_key)
    print("处理完成，从 episode0 开始，共转换条数 =", begin)
